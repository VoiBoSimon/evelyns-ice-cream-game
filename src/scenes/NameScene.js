...
export class NameScene extends Phaser.Scene {
  ...
  create() {
    ...
    const inputEl = this.add.dom(width / 2, height * 0.57).createFromHTML(`
      <input id="player-name" type="text" maxlength="16" placeholder="Enter your name..."
        autocomplete="off" autocorrect="off" autocapitalize="words" spellcheck="false"
        style="font-size:22px;font-family:Arial,sans-serif;padding:10px 18px;border-radius:10px;
               border:3px solid #ec4899;outline:none;text-align:center;width:280px;
               color:#be185d;background:#fff;box-shadow:0 4px 12px rgba(236,72,153,0.15);" />
    `)
    ...
    if (inputEl.node) {
      inputEl.node.addEventListener('keydown', (e) => {
